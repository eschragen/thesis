...
video = video %>% select(id, video)
video$video[is.na(video$video)] = 0
df = claims %>% left_join(video, by = "id") %>% select(date, company, environment, video) 
df$posting = 1
df$environment[is.na(df$environment)] = 0
df$video[is.na(df$video)] = 0
df$environment = as.numeric(df$environment)
df$video = as.numeric(df$video)
df$posting = as.numeric(df$posting)

summary(df)
table(df$environment, df$video)
df$environment_media = NA
df$environment_media[df$environment == 1 & df$environment == 1] = 1
df$environment_media[is.na(df$environment_media)] = 0
...
df_cocacola = df_cocacola %>% group_by(date) %>% 
                      summarise(env_post = sum(environment), total_post = sum(posting), total_env_media = sum(environment_media)) 
...
df_cocacola$total_env_media[is.na(df_cocacola$total_env_media)] = 0
...
setDT(df_cocacola)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
for (i in 1:6) {
  ...
  df_cocacola$total_env_media_perweek[i] = sum(df_cocacola$total_env_media[1:i])
  i = i +1
}

#calculate ratio of environmental claims with media support
df_cocacola = df_cocacola %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_cocacola$ratio_env_media[is.nan(df_cocacola$ratio_env_media)] = 0
...
df_nestle$total_env_media[is.na(df_nestle$total_env_media)] = 0
#amount of postings in the last 7 days
setDT(df_nestle)[, total_posts_perweek := frollsum(total_post, 7)]
setDT(df_nestle)[, total_env_posts_perweek := frollsum(env_post, 7)]
setDT(df_nestle)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
for (i in 1:6) {
  ...
  df_nestle$total_env_media_perweek[i] = sum(df_nestle$total_env_media[1:i])
  i = i +1
}

#calculate ratio of environmental claims with media support
df_nestle = df_nestle %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_nestle$ratio_env_media[is.nan(df_nestle$ratio_env_media)] = 0
...
setDT(df_ikea)[, total_env_media_perweek := frollsum(total_env_media, 7)]

i = 1
for (i in 1:6) {
  df_ikea$total_posts_perweek[i] = sum(df_ikea$total_post[1:i])
  df_ikea$total_env_posts_perweek[i] = sum(df_ikea$env_post[1:i])
  df_ikea$total_env_media_perweek[i] = sum(df_ikea$total_env_media[1:i])
  i = i +1
}

#calculate ratio of environmental claims with media support
df_ikea = df_ikea %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_ikea$ratio_env_media[is.nan(df_ikea$ratio_env_media)] = 0
...
df_exxonmobil$total_env_media[is.na(df_exxonmobil$total_env_media)] = 0
#amount of postings in the last 7 days
...
setDT(df_exxonmobil)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
#calculate ratio of environmental claims with media support
df_exxonmobil = df_exxonmobil %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_exxonmobil$ratio_env_media[is.nan(df_exxonmobil$ratio_env_media)] = 0
...
df_hm$total_env_media[is.na(df_hm$total_env_media)] = 0
#amount of postings in the last 7 days
setDT(df_hm)[, total_posts_perweek := frollsum(total_post, 7)]
setDT(df_hm)[, total_env_posts_perweek := frollsum(env_post, 7)]
setDT(df_hm)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
#calculate ratio of environmental claims with media support
df_hm = df_hm %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_hm$ratio_env_media[is.nan(df_hm$ratio_env_media)] = 0
...
setDT(df_shell)[, total_env_media_perweek := frollsum(total_env_media, 7)]

i = 1
for (i in 1:6) {
  df_shell$total_posts_perweek[i] = sum(df_shell$total_post[1:i])
  df_shell$total_env_posts_perweek[i] = sum(df_shell$env_post[1:i])
  df_shell$total_env_media_perweek[i] = sum(df_shell$total_env_media[1:i])
  i = i +1
}

#calculate ratio of environmental claims with media support
df_shell = df_shell %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_shell$ratio_env_media[is.nan(df_shell$ratio_env_media)] = 0
...
setDT(df_unilever)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
for (i in 1:6) {
  ...
  df_unilever$total_env_media_perweek[i] = sum(df_unilever$total_env_media[1:i])
  i = i +1
}

#calculate ratio of environmental claims with media support
df_unilever = df_unilever %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_unilever$ratio_env_media[is.nan(df_unilever$ratio_env_media)] = 0
...
setDT(df_vw)[, total_env_media_perweek := frollsum(total_env_media, 7)]
...
#calculate ratio of environmental claims with media support
df_vw = df_vw %>% mutate(ratio_env_media = total_env_posts_perweek/total_env_media_perweek)
df_vw$ratio_env_media[is.nan(df_vw$ratio_env_media)] = 0
